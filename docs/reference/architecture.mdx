...
| Analyst   | `workers/analyst`   | Conversations and monthly briefings kept in a Durable Object's SQLite, written in the background from reads of the API.         |
...
The API is the only Worker that writes financial records and the only one with a database binding. The processor reads R2 and calls the API. The analyst keeps conversations and briefings in its Durable Object's SQLite and reads financial data only through the API. The web app's analyst screens and the overview's briefing call the Analyst Worker through its binding.
...
4. The web app's server functions validate browser input with the contracts schemas and call the API and the analyst through their typed bindings. `WorkerClient` in `infra/src/worker-client.ts` types a binding's methods as the Worker behind it declares them, plus the `RpcCallError` a call fails with when that Worker throws, runs out of CPU, restarts, or loses the connection. `ApiClient` in `infra/src/api.ts` is the API's, and `AnalystClient` in `infra/src/analyst.ts` the Analyst Worker's. The web maps that error to `unavailable` when it answers a request, and the analyst when a call returns (`workers/analyst/src/platform/api.ts`). `apps/web/src/server/api-client.server.ts` exposes `ApiClient<WebOperation>`, and `apps/web/src/server/analyst-client.server.ts` exposes `AnalystClient`. `WebOperation` is every operation except `InternalOperation`, the ones only the processor, the analyst, and tests call, such as `publishImport` and `recordModelUsage`. `AnalystOperation` names what the analyst may call: reads, previews, `getModelAllowance`, and `recordModelUsage`. Naming any other command there fails to compile.
...
The analyst answers questions and writes briefings in the background too, through the object's alarm instead of a Workflow, and an hourly cron queues last month's briefing. See [Analyst conversations](#analyst-conversations) and [Analyst briefings](#analyst-briefings).

## Analyst conversations

One `Conversations` Durable Object holds every conversation and briefing, because they all describe the one ledger. It is an Effect layer like the API, and calls the model through Effect AI (see [the decision](/reference/decisions#the-analyst-is-effect-ai-in-a-durable-object)). It stores both in its SQLite through `@effect/sql-sqlite-do`. `workers/analyst/src/storage/migrations.ts` holds the schema as `effect/unstable/sql/Migrator` migrations, one statement per `sql` call, and JSON columns hold the contract's JSON encoding as text.
...
| `proposals`     | The changes an answer proposed, in order, each with its intent, preview, title, reason, and status: pending, accepted, ignored, or stale.                        |
| `briefings`     | Each month's briefing with its status, and once written, its sections, the figures and limits they show, and the fingerprint of its facts, or why it failed.     |

A turn is `queued`, `running`, `answered`, `blocked`, or `failed`. `blocked` means the analyst is off or model usage reached the warning, and `failed` that it could not finish. Both keep a message that says why. The `Turn` contract is a union on `status`, so only an `answered` turn has an answer, only a `blocked` or `failed` one has a message, and only a finished one has `finishedAt`. `readConversation` decodes each row into the member of its status.
...
`ask` stores the queued turn and sets the object's alarm. The alarm runs `WorkScheduler.runNext`, which works on one item and sets the alarm again while turns or briefings wait, so the answer does not depend on the browser staying open. A waiting turn comes before any briefing, because someone is waiting for its answer. A turn that finds the analyst off, or usage at the warning, ends `blocked` with the allowance's message. Otherwise the analyst works with the model until it has an answer it can keep, and the turn ends `answered` or `failed` (see [the analyst's turns](/reference/ai#turns)). Each step is stored as the analyst takes it, so the conversation shows the steps of a running turn.

A failed alarm runs again, up to six times with backoff. A turn an earlier attempt left `running` starts again, and each start counts an attempt on the turn and clears the steps of the attempt before. A turn found `running` after five attempts ends `failed`, so it never stays running or holds up other conversations. A briefing's write is counted and ended the same way, with "The analyst could not finish this briefing. Write it again." A write whose call to the API fails `unavailable` fails the run too, so the platform writes it again once the API answers, and the attempt counts. The count is the turn's or the briefing's, not the platform's retry count, because every ask sets the object's one alarm again and starts that count over.

## Analyst briefings

The `briefings` table holds each month's briefing as `queued`, `writing`, `ready`, `blocked`, or `failed`. `getBriefing` shows `queued` and `writing` as `writing`. A write that finds the analyst off, or usage at the warning, ends `blocked` without a message, and `getBriefing` gives the allowance's reason when it is read. Queuing a write clears what an earlier write left, and `attempts` counts the starts of the write since. The alarm first writes a briefing an earlier run left `writing`, then the latest queued month (see [the analyst's briefings](/reference/ai#briefings)). Whatever queues a write sets the alarm next, with no other I/O between them, so the object commits both together.

The Analyst Worker registers a cron trigger with `Cloudflare.Workers.cron("0 * * * *", …)` and provides `Cloudflare.Workers.CronEventSourceLive`, so each deploy attaches the expression to the Worker. Alchemy runs the handler only for a scheduled event whose `cron` is exactly `0 * * * *`, and drops a failure without a trace, so the handler logs its own failure. At the top of every hour, the handler calls the object's `queueLastMonthsBriefing`. It takes the calendar month before the one containing today in the settings timezone, and queues its first write when the month has no briefing yet, of any status, and `requestBriefing` could queue it:

- `hasRecords` in `packages/finance` finds records in the month from `getCoverage`,
- `getModelAllowance` allows the task `briefing`, and
- `getFactsStatus` counts no outdated events.

A run that finds any of these false queues nothing, and the next hour's run checks again, so last month's briefing is queued in the first hour they all hold. The cron never queues a month with a briefing again. `getBriefing` writes it again when its facts change, or once the analyst may call its model after a blocked write, and `requestBriefing` when you ask. The local runtime fires the cron on the same schedule, and `GET <Analyst Worker URL>/cdn-cgi/handler/scheduled?cron=0+*+*+*+*` fires it at once. Without the exact expression in `cron`, the handler ignores the event.

## Infrastructure ownership

Each service module under `infra/src/` declares the resources that service owns. The API in `infra/src/api.ts` declares Postgres, Hyperdrive, the sources bucket, and the `TemporaryExports` bucket with its seven-day expiry. The processor in `infra/src/processor.ts` declares four Workflows: `ImportWorkflow`, `ExportWorkflow`, `EnrichmentWorkflow`, and `FactsWorkflow`. The web module declares the site, domain, and Access application. The analyst in `infra/src/analyst.ts` declares the `Conversations` Durable Object class, the Analyst Worker that hosts it and its hourly cron trigger, and its own AI Gateway, `AnalystGateway`. Deployment values and secrets stay in the stack and never enter a runtime bundle. Runtime entry points may import their inferred binding types from `@repo/infra/worker-bindings` as type-only imports.
