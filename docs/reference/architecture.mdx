...
2. `infra/src/api.ts` declares the API Worker class with `ApiOperations`, and `infra/src/processor.ts` picks the processor's methods with `Pick`. The bridge infers argument, success, and tagged failure types for callers.
...
4. The web app's server functions validate browser input with the contracts schemas and call the API through the typed binding. `apps/web/src/server/api-client.server.ts` exposes `WebOperation`: every operation except `InternalOperation`, the ones only the processor and tests call, such as `publishImport`.
