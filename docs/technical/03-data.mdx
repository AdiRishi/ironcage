...
- Every displayed figure in the product is recomputable from Postgres rows and any artifacts its owning domain explicitly retains.
...
| **R2**                   | Tax source evidence, candle archives, backtest artifacts, report bodies, proposal bundles, logical dumps                                                                                              | Blobs retained only under an explicit owning-domain policy. Money import uploads are excluded.                      |
...
- **R2 by reference.** When a domain retains an R2 blob, Postgres holds its key and the key embeds the owning row's ID. Money imports persist source metadata and observations without retaining upload blobs.
