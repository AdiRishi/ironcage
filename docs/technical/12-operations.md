...
The venue key posture is specified in [Venues](./06-venues.md). Kraken permissions are asserted from the API. Alpaca publishes no equivalent scoped-key or documented account-level crypto-disable guarantee, so its transfer surface remains a launch gate rather than a claim in this table.
...
The database story has three lanes. Ordinary local development uses Hyperdrive's `localConnectionString` to connect directly to a PlanetScale dev branch; that is convenient but explicitly bypasses Hyperdrive pooling and caching. CI uses Postgres 18.4 in Testcontainers, with one container per database test file and a drop-schema → migrate cycle before every test, so pipelines are hermetic, parallel-safe, and exercise a cold schema. A deployed dev Worker runs the same integration suite through real dev Hyperdrive configurations before any database-sensitive change can merge. All three lanes use the same migrations and public contract tests.

The backtest container needs a local Docker daemon. Work that does not touch backtests may skip it.

## 4. Configuration and secrets

Secrets are Worker secrets set through Wrangler, except the dump credential, which belongs to the backup workflow. They must not appear in code, in files, or in the database. A Wrangler secret update creates and deploys a Worker version immediately, so production secret creation or rotation is a release operation: the root workflow blocks entries first, applies the secret change, redeploys and verifies the entire repository, then unblocks. Direct operator `wrangler secret put` in production is not an allowed path.
...
New Durable Object classes use current declarative `exports`. Workers that own them deploy with `wrangler deploy`, because the versions-upload path is incompatible with those exports ([Durable Object migrations](https://developers.cloudflare.com/durable-objects/reference/durable-objects-migrations/)).
...
1. **Build and test the exact commit.** Typecheck, lint, every suite in section 6 except `venues/live`, generated Wrangler configuration, and image builds must pass. Only one production workflow may run at a time.
2. **Block new entries.** The workflow calls the recorded system-wide entry-block operation and verifies it. Exits, stops, fill ingestion, ambiguity resolution, and reconciliation continue.
3. **Apply the additive migration.** The migration runs under a Postgres advisory lock and records checksum, tool version, start time, outcome, and verification query. Code remains compatible with both the old and expanded schema.
4. **Deploy the whole release.** One root `pnpm deploy` freezes new compute dispatch and publishes the immutable image. It invokes `wrangler deploy` for every Worker in a fixed dependency order. The same command applies the release's secrets, routes, domains, Cron triggers, bindings, DO exports, and immediate container update. It waits for image readiness before proceeding. The operator never selects a subset. An unchanged Worker may be rebuilt and redeployed; the small extra work keeps one release path.
5. **Verify the whole system.** One release check asserts the signed health route and commit/release IDs, an `AppApi` decode, vitals, feed reconnect/replay, current DO export set, Cron/route configuration, container image identity, agents' gateway-only provider URLs, and the app shell. It also runs the protection pass and confirms no obligation was lost during restarts.
6. **Record and unblock.** A single `System` feed event records the commit, release ID, version IDs, container identity, migration, entry-block duration, and verification result. Only a fully verified release lifts the block.
7. **Failure posture.** Any failure leaves new entries blocked. The default is fix-forward from `main`. When the previous commit remains schema/binding compatible, the same whole-system command redeploys that commit; the pipeline never rolls back only one Worker. External state—Postgres, R2, DO storage, queues, and venue facts—is never rolled back.
8. **Contract later.** A destructive migration ships in a later whole-system release no earlier than **7 days** (proposed) after every reader and writer stopped using the old representation and a backup was verified.
...
**R2 protection is layered.** Content-addressed keys and bucket locks prevent an object-write credential or ordinary tooling accident from overwriting/deleting protected objects. A bucket-administration credential can remove lock rules, so it is isolated from every Worker, lock-rule changes alert externally, and PlanetScale remains the independent failure domain. R2 has no native object versioning; nothing here assumes it.
...
| R2 object operations are strongly consistent; lock rules stop object writes/deletes but can be removed by bucket administrators; no native object versioning exists                                                       | [Consistency](https://developers.cloudflare.com/r2/reference/consistency/), [Bucket locks](https://developers.cloudflare.com/r2/buckets/bucket-locks/)                                                                                 | runtime has object-only authority; release check asserts rules; admin changes alert          |
...
| New Durable Object projects use declarative exports; `versions upload` is incompatible with exports; `wrangler deploy` applies the Worker release                                                                         | [DO migrations](https://developers.cloudflare.com/durable-objects/reference/durable-objects-migrations/), [Wrangler config](https://developers.cloudflare.com/workers/wrangler/configuration/)                                         | whole-release scratch deploy, failure, and previous-commit redeploy                          |
...
- **Version upload, per-Worker promotion, and percentage rollout.** Rejected: declarative DO exports are incompatible with versions upload, and partial rollouts add operational states without value for one operator. One `wrangler deploy`-based root command deploys every Worker.
...
- [ ] Root `pnpm dev` running four processes; direct local Postgres, remote R2 where useful, and a separate deployed Hyperdrive integration lane
...
- [ ] Backup workflow: trusted backup-profile `pg_dump` under the read-only `DUMP_DSN`, writing to `backups/{yyyy-mm-dd}/` in a bucket-locked R2 bucket
