...
1. **Build and test the exact commit.** Typecheck, lint, every suite in section 6 except `venues/live`, the Alchemy stack, and image builds must pass. Only one production workflow may run at a time.
...
3. **Review the infrastructure plan.** `pnpm plan` evaluates the whole Effect program and shows database migrations and resource changes together. Code remains compatible with both the old and expanded schema.
4. **Deploy the whole release.** One root `pnpm deploy` reconciles the Alchemy dependency graph. PlanetScale migrations run before their dependent Hyperdrive and Worker resources; the same command applies secrets, routes, domains, Cron triggers, bindings, DO exports, Queues, storage policy, and container updates. The operator never selects a subset.
