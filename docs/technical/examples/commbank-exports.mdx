...
This note records the NetBank behavior observed on 8 August 2026 and turns it into a repeatable collection procedure for Money. It is empirical evidence for [Money](/technical/money). Production parser contracts still require redacted byte-preserving fixtures.
...
AnyDoc runs in an isolated compute image with network access disabled. The image is pinned to the exact evaluated version. Raw PDFs remain authoritative. When statement parsing is enabled, confirmed imports must retain the extracted Markdown, exact AnyDoc version, and deployed image digest for reproducibility.
