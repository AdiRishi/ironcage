---
title: CommBank export evidence
description: The NetBank behavior observed on 8 August 2026, turned into a repeatable collection procedure for Money.
sidebar:
  label: CommBank exports
  icon: landmark
---

This note records the NetBank behavior observed on 8 August 2026 and turns it into a repeatable collection procedure for Money. It is empirical evidence for [Money](/technical/money). Production parser contracts require structure-preserving fixtures and private-corpus validation where raw evidence cannot be committed.
...
The date cell is unquoted; the amount, narrative, and balance cells are quoted. Positive amounts and positive running balances carry an explicit `+` prefix.
...
- Mastercard purchase narratives are fixed-width padded merchant, city, and state columns. The internal space runs are significant and must survive parsing, because pairing compares the narrative against the OFX `MEMO` byte for byte. The observed payment row is plain text without padding.
...
`DTUSER` carries the value date and often differs from `DTPOSTED` — 22 of the 40 rows in one spending-offset sample. Where the narrative prints a `Value Date`, the `DTUSER` digits match it. The observed `TRNTYPE` values are only `DEBIT` and `CREDIT`. `TRNAMT` prints credits without a sign while the paired CSV amount carries an explicit `+`, so row pairing compares numeric amounts rather than amount text.
...
Observed rendering: amounts print without currency symbols and with thousands separators, balances carry a `CR` suffix, and transaction dates are a zero-padded day and abbreviated month without a year. The opening and closing balances appear as sentinel rows inside the table with a full date, and a final summary row proves opening − total debits + total credits = closing. Statement narratives render `Value Date` without the colon that the structured exports include.
...
The older-history collection procedure is:

1. Open **Statements** from NetBank's account navigation.
2. Select one account in scope and one available financial year.
3. Download one listed statement PDF for the spending offset.
4. Upload it to the statement import flow while the file remains available from NetBank.
5. Verify the preview's account, statement window, opening and closing balances, row counts, and overlap effects before confirming.
6. Repeat for every available supported offset statement.

Mastercard and home-loan statements are not accepted by the offset profile. Each requires its own representative evidence and fixture-gated parser before its older history can be imported.

## AnyDoc evaluation

[AnyDoc](https://github.com/firecrawl/anydoc) 0.1.9 was evaluated locally against the private sample corpus.
...
AnyDoc runs in an isolated compute image with network access disabled. The image is pinned to the exact evaluated version. PDF bytes and extracted Markdown are transient; confirmed imports retain the input digest, source metadata, extractor identity, and row-level observations.

No hosted OCR service is part of the initial design. A scanned or extraction-empty PDF fails visibly and is discarded until a separate privacy decision authorizes another extraction path.
...
This investigation closes the choice of recent-history format. The redacted observed corpus covers paired samples for all four account profiles and three overlapping spending-offset windows. It lives at [`apps/core/tests/fixtures/money/commbank`](https://github.com/AdiRishi/ironcage/tree/main/apps/core/tests/fixtures/money/commbank). Deterministic fixtures are sufficient evidence for behavior that does not require another bank export: equal-row occurrence, CSV quoting and character decoding, empty results, and truncation rejection.

Additional statement profiles remain source-dependent:

- Download a representative Mastercard and home-loan statement for parser assessment.
- Verify the oldest layout for each new statement profile because the available window may span multiple templates.
