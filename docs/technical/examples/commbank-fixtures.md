...
The redacted observed corpus lives in the core test tree at [`apps/core/tests/fixtures/money/commbank`](../../../apps/core/tests/fixtures/money/commbank/README.md). It covers all four structured account profiles plus three overlapping spending-offset windows without changing delimiters, quoting, encodings, line endings, empty fields, tag order, or other parser-significant syntax. Deterministic fixtures cover parser and reconciliation cases absent from that corpus. Raw bank files never belong in the repository.
