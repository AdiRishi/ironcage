...
```mermaid
flowchart TB
    U["Order call outcome unknown"] --> H["Preserve the reservation<br/>block risk-increasing intents on the instrument"]
    H --> Q["Query by client order ID over a bounded interval<br/>Kraken: Open + Closed Orders, then Trades/Ledger<br/>Alpaca: order lookup plus immutable Activities"]
    Q -->|"order found"| A(["adopt the venue's state<br/>ingest executions from the venue's<br/>authoritative activity source"])
    Q -->|"query failed —<br/>maintenance, 429, 5xx, transport"| R["retry with backoff<br/>from the actor's alarm<br/>failure never implies non-placement"]
    R --> Q
    Q -->|"repeated negative snapshots"| X(["quarantined — reservation stays held<br/>reconciliation evidence or a recorded<br/>operator resolution is the only exit"])
```

A negative snapshot is a query that succeeded and found nothing — and even that never proves non-placement on its own, which is why repetition leads to quarantine rather than resubmission.
