...
A direct call that fails is an error to handle — with Effect's declarative retry policies — not a reason to turn a synchronous mental model asynchronous. Where a Durable Object must guarantee something _eventually_ happens (flushing feed events to Postgres, sending the halt email), it writes the obligation into its own storage in the same transaction as the work, and its alarm retries delivery: the outbox pattern — the same at-least-once guarantee as a queue, with the pending work inspectable where it belongs. Cloudflare Queues appear in exactly one place: **agents → core delivery of decision records**, where the producer is a stateless Worker with no durable home for an outbox; the queue provides buffering, retries, and dead-letter capture across that seam. The order path is never queued: at-least-once _and unordered_ are precisely the wrong properties for orders.
...
<a name="d20"></a>**D20 — Deployment target: the operator's personal Cloudflare account, at `wealth.arishi.dev`.** A dedicated domain is unnecessary for a private, authenticated, single-operator app; the `arishi.dev` zone lives on the personal account, and the app is scoped to the `wealth` subdomain (which is also what the Access application protects, [D27](#d27)).
...
<a name="d28"></a>**D28 — Build order: Money first, Tax second, the investing engine third.**
Phase 1 is the wealth-management surface (bank import, categorization, spending analysis) — zero execution risk, useful in week one, and it quietly forces the platform spine (workers, database, feed, decision records, reports, the first Flue agent) to exist before anything can lose money. Phase 2 is the tax engine, completing mastery of the read-only half and replacing the paid external service. Phase 3 is the investing engine — actors, cage, workbench — entering through dry run per the lifecycle, with the crypto-trend sleeve as the proving tenant. The full plan with dependency map: [ROADMAP.md](../ROADMAP.md). Dry run uses **our own simulated-fill machinery against live market data** ([Engine](./04-engine.md)) on both venues — not venue paper accounts: Kraken offers no spot sandbox at all, and one simulator serving dry run, backtests, and the no-AI counterfactuals is the identity the evidence system depends on. Alpaca's paper environment is still provisioned — as the venue adapter's integration-test target ([Operations](./10-operations.md)), not as the dry-run mechanism.

## Deferred

<a name="d21"></a>**D21 — Deferred decisions.** Provider/model selection per capability — **TODO**, decided at first capability configuration; current leaning is to start with Cloudflare-hosted models (Workers AI catalog, likely a Kimi model), which also makes gateway routing automatic ([D11](#d11)). Also deferred: any notification channel beyond email (D19); streaming market data (D7's recorded re-entry path). _(Auth was deferred here and has since been decided — [D27](#d27).)_
