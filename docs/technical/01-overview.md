...
**`ironcage-core` is written in Effect V4** ([D1](./00-decisions.md#d1)): every subsystem is a service with a typed interface, wired by layers; every boundary — venue responses, config, AI outputs, order intents — is an Effect Schema; failures are typed and handled, retries are declarative. The engine's defining property is that it is _deterministic_: given the same recorded inputs, it does the same thing, which is what makes the blotter recomputable, backtests honest, and audits possible.
...
- **Capability ticks.** Agents run on their own cadences in `ironcage-agents`, persist their outputs and decision records, and are _read_ by the next sleeve tick. AI is never awaited inside the order path.
