...
flowchart TD
    ...
    Dispatcher --> Dispatch[Queues]
    Dispatch --> Jobs[Processor Workflows]
...
Alchemy owns runtime declarations, PlanetScale resources, Hyperdrive bindings, migrations, local wiring, and deployment. Effect owns service composition, typed failures, and cancellation inside each runtime. Worker RPC derives its method contracts from the service declarations.
...
A financial command commits its data change and a dispatch record in one Postgres transaction. The API's dispatcher claims pending records with a bounded lease, then sends queue messages after committing the claim. A failed or ambiguous send leaves recoverable work. Duplicate delivery returns the existing job or committed result.
