...
The processor's classification service owns an AI Gateway in
`infra/src/classification.ts`. `infra/src/worker-bindings.ts` composes its
`QueryGateway` binding into the classification workflow. GLM inference uses
the AI SDK's Workers AI provider with the bound gateway ID. Authentication is
enabled; gateway logging and caching are disabled. Development uses a real
stage-specific gateway. Local infrastructure tests replace only the gateway's
cloud resource provider and do not make model calls or create cloud resources.
