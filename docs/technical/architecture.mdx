...
Stage 1 builds web/API/processor, financial storage, evidence, dispatch, and backups. Stage 2 adds document extraction infrastructure; stages 3 through 5 add interpretation and financial analysis; stage 6 adds the analyst and computation; stage 7 adds proactive work. The graph below describes the eventual system. Instantiate only the selected stage's resources.
...
flowchart TD
    ...
    Jobs -->|scoped conversation export| Analyst
    API --> Maintenance[Fixed-command backup Container]
    Jobs --> Gateway
...
Only the public web Worker accepts browser traffic. It authenticates SDK HTTP and WebSocket requests and routes directly through the analyst-owned Durable Object namespace. Alchemy declares a cross-script namespace reference in the web Worker; the analyst Worker owns the class and migrations and deploys first. Routing derives the instance name from the authenticated owner and conversation ID. The namespace capability is limited to the analyst and is never selected from arbitrary browser input. The web Worker replaces private identity headers with verified Access identity; the analyst binds that context to the connection before acceptRun, as defined in Shared contracts. This removes the intermediate Worker HTTP hop while retaining ingress and reconnect tests. Other runtimes receive capability-specific bindings. The API holds financial write capabilities. The analyst receives financial query and command tools rather than direct storage access.
