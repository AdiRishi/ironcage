...
Stage 1 implements CSV/OFX extraction, matching, publication, source review, and coverage. Stage 2 adds PDF, full-corpus validation, and source removal. Stage 3 adds interpretation; stages 4, 5, and 7 add refresh consumers. Posting import succeeds independently of all later interpretation and analysis.
...
Stage 1 handles every supplied CSV and OFX. Stage 2 extends the same pipeline to every supplied PDF. File identity, transaction identity, and financial relationships are separate decisions. A repeated file is not the same problem as a transfer recorded in two accounts.
...
A reupload of removed bytes restores source access under the same identity after verification. Withdrawn records stay withdrawn until the user accepts an undo of the withdrawal. An upload during removalPending can retry after removal completes. The [removal and restoration protocol](/technical/import-protocol#refresh-and-removal) defines version fencing.
...
The upload path supports every supplied format assigned to the current stage. Operational byte and page limits are explicit, displayed before upload, and tested against the corpus. Larger files are streamed and processed in bounded chunks.
...
<Tabs>
  ...
  <Tab title="PDF text">
    The processor uses unpdf's serverless PDF.js build to extract page text and positions. Its
    CommBank layout adapter separates transaction tables from totals, rewards, notices, and repeated
    page headers. Debit and credit signs follow the statement layout.
  </Tab>
...
Parser and extraction versions are recorded per observation. An improved parser can reprocess original files without replacing user corrections. Text and visual extraction share the same output contract. The Container rendering build is pinned after a rendering check against the supplied corpus.
...
The processor owns the PDF text adapter and uses [unpdf](https://github.com/unjs/unpdf). The evaluated baseline is `unpdf@1.8.1`, whose bundled PDF.js engine reports `6.1.200`. The adapter reads positioned text items page by page through `getDocumentProxy` and PDF.js `getTextContent`, with text normalization disabled. The CommBank adapter owns row grouping, column interpretation, and source locators.

The September 2026 comparison covered five supplied PDFs, 25 pages, and 303 manually verified transaction rows. With a layout decoder and a visually established repair of missing card decimal separators, unpdf matched every transaction date and signed amount and all 258 available running balances. Python native extraction did not improve these results, so the selected path does not add a Python extraction or OCR dependency.

Native PDF text can omit decimal separators in card amounts and foreign-currency descriptions. Ambiguous monetary strings use the existing rendered-page document extraction flow. A deterministic repair requires a versioned source profile evaluated against the visible statement layout; matching a statement total alone does not authorize inserting decimal points. Raw extracted strings remain attached to the observation.

The processor pins the library and bundled engine versions with its extractor version. Upgrades require the [extraction evaluation](/technical/verification#extraction-and-interpretation-evaluation). The comparison establishes the text-library choice. Implementation acceptance still requires full-corpus validation, hosted Worker resource checks, and Container rendering checks.
...
A Workflow stages extraction and matching against an immutable financial snapshot. The API commits bounded groups of validated postings, observation links, review items, and dispatch records as commands under the [financial command protocol](/technical/financial-model#persistence-and-concurrency). If the input revision changed, matching repeats against current records, and remaining groups are matched against the resulting snapshot before publication.

Unique source-observation constraints and revision checks make simultaneous overlapping imports converge to the same history. Batch summaries derive from committed results, not from attempted insert counts. Extraction, model interpretation, and source-object writes finish outside the financial transaction.

## Enrich and refresh

From stage 3, deterministic financial-role establishment runs before semantic enrichment, following [Financial-role establishment](/technical/import-protocol#financial-role-establishment). Known merchant rules and explicit corrections run before model classification. Only novel or unresolved interpretations require model calls. Accepted amount and date evidence remains separate from semantic proposals.

After posting publication, finalizeExtraction publishes supported statement anchors and source coverage. Stage 4 then refreshes affected period summaries and saved analyses. Stage 5 adds recurring-cost detection, occurrence matches, and forecasts. Stage 7 adds insights and intentions. Stages 1 and 2 complete after posting publication, coverage finalization, and the import summary. [Processor RPC](/technical/rpc#processor-only-rpc) defines their read and publication operations. Jobs include their input revision and calculation version. A late job cannot replace newer results.
