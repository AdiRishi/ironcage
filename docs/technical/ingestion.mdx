...
<Tabs>
  <Tab title="CSV">
    The CommBank parser handles the observed headerless date, signed amount, description, and
    balance columns. It preserves empty card balances, quoted fields, byte-order marks, and line
...
The score is a rule score, not a calibrated probability. Thresholds are selected against checked duplicate and non-duplicate examples. Accepted matches retain the evidence that justified them.
...
A Workflow stages extraction and matching against a captured dataset revision. The API commits bounded groups of validated postings, observation links, review items, and dispatch records in D1 transactions.

Every group has a stable commit ID. If a commit returns no response, the caller retrieves the recorded outcome before retrying. If another import changed candidate matches, publication rejects the stale expectation and matching repeats against current records.
...
Known merchant rules and explicit corrections run before model classification. Only novel or unresolved interpretations require model calls. Accepted amount/date evidence remains separate from semantic proposals.
