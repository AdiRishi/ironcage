...
After posting publication, finalizeExtraction publishes supported statement anchors and source coverage. Jobs then update recurring-cost detection, occurrence matches, affected period summaries, saved analyses, forecasts, and insights. [Processor RPC](/technical/rpc#processor-only-rpc) defines their read and publication operations. Jobs include their input revision and calculation version. A late job cannot replace newer results.
