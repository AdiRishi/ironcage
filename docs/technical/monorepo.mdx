...
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `apps/web`          | Authentication at browser entry, TanStack Start server functions, routes, query state, shadcn views, and the conversation proxy.                                         | Public Worker and domain. No financial database or independent financial cache.                                                                                   |
| `workers/api`       | Financial commands and queries, source registration, evidence access, snapshots, review, revisions, dispatch, and the application records in the database specification. | PlanetScale Postgres, Hyperdrive, source and artifact R2 buckets, and backup resources. Owns migration execution requirements and retention policy.               |
| `workers/processor` | Import extraction, reconciliation preparation, interpretation, numerical analysis, refresh, export, removal, and bounded computation jobs.                               | Dispatch queue and dead-letter queue, Workflows, and the computation Container when required by the specified workload. Workflow state owns execution scheduling. |
...
apps/web/src/
  ...
  server/                     authentication, RPC client, stream proxy
workers/api/
  src/
...
workers/processor/src/
  imports/                    CSV/PDF extraction and reconciliation jobs
  ...
  platform/                   Workflow, queue, API, and model composition
  index.ts
workers/analyst/src/
...
infra/src/
  ...
  processor/                  queues, Workflows, computation resources
...
Each workspace keeps its own tests. The API's financial operations use concrete repositories and the shared transaction protocol. A repository groups SQL for a domain operation; it is not a generic CRUD interface for each table. Internal query and command modules can use the same transaction-scoped database layer without calling each other over RPC.
...
Workers communicate through bindings, HTTP streams, or queue messages defined in the specifications. A Worker does not import another Worker's repository, runtime implementation, or private module. Shared packages cannot import from `apps`, `workers`, or `infra`.
...
Schemas validate browser input, imported bytes, model output, stored serialized payloads, and externally supplied HTTP or queue envelopes. Decoded values pass through internal functions without repeated shape validation. Financial ownership, expected versions, and the remaining amount eligible for a credit link are command preconditions checked against current state even when the input is well typed.
