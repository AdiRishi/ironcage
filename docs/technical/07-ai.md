...
- Every consequential AI output has a permanent decision record: what was asked at decision grain, what was decided, the stated rationale, model and configuration version, timing, cost, and the gateway span IDs.
...
1. **Dispatch.** The agents Worker's cron fires on the capability's cadence. The run ID is derived deterministically: the run identity `(capability, config_version, cadence_ts)` is hashed into a name-based UUID (UUIDv5-style over the run identity), where `cadence_ts` is the scheduled time on the capability's cadence grid. A duplicate cron firing re-derives the same ID and re-enters the same run. This is the one deliberately deterministic ID in the system; idempotent re-dispatch requires that the same inputs yield the same identity, which a random time-ordered ID cannot provide. Core validates on arrival that `cadence_ts` lies on the capability's grid and is not in the future; an off-grid identity is rejected as invalid.
2. **Context.** The agent reads exactly what its `consumes` declaration names, through `AgentReadApi` and nowhere else. What it read is summarized into the decision record at decision grain.
3. **Inference.** The model and parameters come from the capability's configuration row. For the regime assessment the asked shape is: per instrument, the named axes on the quantized ladder `{0, ¼, ½, ¾, 1}`, each with a one-sentence rationale and cited source records.
4. **Wire validation.** The agent validates the answer against the output schema at its own boundary, retrying malformed answers up to **2 times** (proposed). Still malformed → the run is a **failed run**, and the failure itself is the delivery.
5. **Delivery.** One queue message carries the output (or the failure) and the decision record. The queue is the agents Worker's only write path; it holds no R2 binding and no Postgres binding. A message must fit the 128 KiB queue budget; an output that exceeds it is a failed run at v1. A staging-bucket pattern for larger outputs is recorded as a future option, not built. The queue contract itself — at-least-once delivery, individual acks, dedupe by run ID inside the write transaction, dead-lettered messages surfacing as a `decision_record_lost` warning — is defined once in [Contracts](./04-contracts.md).
6. **Acceptance.** Core's consumer re-validates with the authoritative Schema from `packages/contracts`, regardless of what the agent checked. It then writes, in one Postgres transaction: the `capability_outputs` row with `valid_until` precomputed, the decision record, and the feed event (`notice` when the registry's material-change rule says the output changed something, `warning` for a failed run). A duplicate run ID inside that transaction is a no-op. The same run ID arriving with a different payload hash is not a duplicate; it is an invariant violation and raises a critical event. Where a capability's output includes an aggregate derived from its own parts — the regime assessment's combined entry multiplier derived from its axes — core recomputes the aggregate from the parts and rejects a mismatch.
7. **Consumption.** The next tick reads the newest valid row, defined precisely: among rows that are accepted, unexpired at the tick's logical time, unsuspended, and matched to the current mandate and configuration, the tick consumes the greatest `(scheduled_at, run_id)`. Stale, absent, or failed resolves to the safe default. A failed AI run may never delay, block, or alter a tick.
...
A capability output's lifetime anchors on when the run was scheduled, not on when the model happened to finish. This matters because a slow provider must not be able to stretch an assessment's relevance.

- `valid_until = scheduled_at + registry.validFor`. The default `validFor` is 1.5 × cadence (proposed). A mandate may shorten the window for its sleeve, never lengthen it.
...
**The decision record is ours and permanent.** For every run — including failed runs — core stores: what was asked, at decision grain, including a summary of the material inputs; what was decided; the stated rationale; the model and configuration version; timing; cost; and the AI Gateway log and span IDs. This record is written in the same transaction as the output row, so an output cannot become visible without its decision record.

**Everything deeper is the platform's and temporary.** Full prompts, context payloads, tool calls, and turn-by-turn transcripts live only in AI Gateway and the platform's observability surfaces, for their retention window. They are queryable by our span IDs while they last, and they expire. We never store them permanently, for any class. The trade proposer is no exception: its gateway retention can be configured longer than other capabilities', but that is a gateway setting, not our storage. Expiry of a platform trace does not break the audit path; the decision record renders on its own, and the UI shows the deep link only while the platform still has something behind it.

The product promise this implements is accountability of decisions, not archival of transcripts. Every consequential answer can be explained from our own rows forever; the raw exchange is a debugging aid with a shelf life.
...
Each call carries metadata: capability name, run ID, config version. That is what makes the gateway's cost accounting per-capability — the number the weekly review's "is the AI worth it?" verdict and Portfolio's cost display read. Per-capability budgets from `CapabilityConfig` are enforced twice: the agent checks before running, and gateway spend caps back it. A budget-exhausted run is a failed run: visible, safe-defaulted, never silent.
...
One interaction is worth naming. The operator's break-glass override may pierce an AI-written lock; the operator outranks the AI, and the ceremony, cooling-off, and single consumption preserve the evidence trail. An override is additionally voided by any new mandate version for the sleeve, bounded by a stated maximum action size, and consumed by compare-and-set in the verdict's transaction. The override ceremony itself is specified in the product spec's operations chapter ([product/07](../product/07-operations.md)) and its consumption in [The tick](./05-the-tick.md).
...
No entry of this class ships in v1. The class is fully specified so that the quarantine shape is designed before anything inhabits it. Its evidence follows the same rule as every class: decision record plus gateway span IDs, nothing more stored by us.
...
Scorecard deltas accrue per capability per evaluation window:

```
pnl_delta(W)      = net_pnl_actual(W)      − net_pnl_identity(W)
drawdown_delta(W) = max_dd_actual(W)       − max_dd_identity(W)
cost_delta(W)     = costs_actual(W)        − costs_identity(W)     (fees + AI spend)
```
...
Money's transaction categorization reuses the capability runtime — the same run identity, queue path, authoritative validation, and decision records — but it is not a sleeve capability. It has no sleeve, no mandate, and no baseline book; it lives in Money's own workflow and its registry entry is Money-owned. Operator-created rules short-circuit the model: a rule-matched transaction never reaches an agent. Automatic application of model categorizations is off at launch; it turns on at a confidence threshold derived from the operator's correction corpus, and that threshold is owned by [Money](./08-money.md).
...
- **Their output is a proposal bundle**: the mandate diff or strategy variant, rationale, and supporting analysis. The bundle travels through the same decision queue as every capability output, within the same 128 KiB budget; an oversize bundle is a failed run at v1. Core validates the bundle and stores it content-addressed in the R2 quarantine bucket, then creates the proposal row. An agent cannot advance its own proposal's state; only the gate pipeline and the operator write those transitions ([Workbench](./10-workbench.md)).
- **The operator can talk to them.** The Workbench UI mounts a conversation over the app Worker's direct binding to the agents Worker; conversations never route through core ([App](./11-app.md)). The binding carries conversations only, and the agents Worker has no mutation path behind it. Conversations are Flue transcripts, which makes them platform-retained debugging data like any other transcript; anything that matters graduates to a proposal or a decision record.
...
| Platform trace expires               | Decision record renders alone; deep link disappears                                           | No behavioral effect                                |
...
| Queue message budget                            | 128 KiB; oversize output = failed run                                                                    | platform limit (contract in ch. 04)           | decided                     |
...
| Categorization auto-apply                       | off at launch; threshold later from the correction corpus                                                | operator (Money, ch. 08)                      | decided                     |
...
- **Storing full prompts, context payloads, or transcripts ourselves.** Rejected for every class: the permanent record is the decision and its reasoning, linked by span IDs into the platform's retention window; transcripts are the platform's debugging layer. The trade proposer gets a longer gateway retention, configured at the gateway, not a private archive.
...
- [ ] Gateway-fronted provider registration, the build-failing direct-URL test, and the caching-disabled assertion for capability calls
- [ ] Deterministic run-identity derivation, duplicate-dispatch re-entry, and core-side cadence-grid validation
- [ ] Queue consumer: authoritative validation, one-transaction writes, run-ID dedupe, payload-hash-mismatch critical event, dead-letter → `decision_record_lost`
...
- [ ] Decision-record-only evidence: schema has no prompt, context-payload, or transcript columns; rendering tested after platform trace expiry
