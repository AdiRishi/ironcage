...
This runtime belongs to stage 6. Earlier document and category model calls use the shared execution package and processor, without conversation storage or analyst routing. The structured extraction/model-policy discussion links to those earlier owners; it does not make this runtime their dependency.
...
sequenceDiagram
    ...
    Agent->>API: Scope, coverage, and snapshot captured at execution
...
Run limits cover model steps, elapsed time, tokens, and spend. Enforcement uses recorded usage and reservations for in-flight calls. A stopped or exhausted run preserves completed evidence and has an explicit terminal state. Provider attempts have separate cost reservations within one persisted logical-call budget. Workflow replay does not reset those attempts.
