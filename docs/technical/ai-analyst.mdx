...
Each tool response identifies the data revision and query scope. When an investigation needs a new revision, it restarts the affected calculation explicitly. It does not combine values from different snapshots without saying so.
...
A server tool can finish a database write before its result reaches the conversation. Recovery looks up the command's recorded outcome before repeating it. A provider interruption can require another model call. Buffered text replay does not resume arbitrary JavaScript execution.
