...
flowchart TB
  ...
  subgraph cf["Cloudflare (personal account)"]
    ...
    r2[("R2<br/>blobs · exports · proposal-bundle quarantine")]
  end
...
| core → compute        | Durable Object binding           | Workflows start backtest runs; the dormant statement-parser design has an isolated extraction seam                                                                                   |
...
**Statement-extraction container** — a separate container profile in compute. It runs pinned AnyDoc against PDF bytes and returns derived Markdown plus the exact extractor identity. It has no database credential, no general egress, and no authority to interpret financial rows. Phase one packages and isolates this native boundary but does not call it; core will perform financial validation only after the statement fixture gate passes ([Money](/technical/money)).
