...
| Money         | Account registration and reads; import preview, confirm, statement archive, history, and coverage; transaction drill-down; category, rule, review-queue, and owned-transfer operations; balances, analysis, and monthly spending reports | Preview and confirm resend source bytes; analysis carries coverage status; every mutation has a request ID |
