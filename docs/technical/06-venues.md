...
This chapter specifies each adapter, the ambiguity protocol, stop management, reconciliation, and the simulated fill model. [The tick](./05-the-tick.md) hands execution a committed intent. This chapter owns its lifecycle after that handoff.
...
2. Fetch the venue's actual state through uncached private endpoints. Kraken spot holdings come from Balance/Extended Balance plus open spot orders and ledger facts; Open Positions is a margin endpoint and is never used as spot inventory. Pending withdrawals/transfers are recorded separately so their exclusion from available balance does not look like unexplained drift.
