...
The import process itself runs in the order of the numbered sections below, and nothing is stored until the last stage commits:

```mermaid
flowchart LR
    U["Upload"] --> S1["§1 Profile<br/>account + source identity"] --> S2["§2–3 Parse<br/>CSV/OFX pair<br/>or statement"] --> S4["§4 Normalize"] --> S5["§5 Dedupe<br/>tiers 0–5"] --> S6["§6 Coverage"] --> S7["§7 Preview → confirm<br/>one Postgres commit"]
```
...
The cascade, end to end — each tier's subsection below carries its full rules:
