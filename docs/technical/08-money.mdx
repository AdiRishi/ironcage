...
The **Needs a hand** ledger can resubmit its unresolved rows to AI. Core excludes transactions already filed by a rule or the operator and submits the rest under the current categorization configuration. An older batch is materialized as a deterministic current-version batch, an unfinished current execution is left running, and a failed current run clears only its failed output before starting fresh Workflow and agent execution identities. The financial run identity stays stable for queue deduplication and audit. Prior decision records and feed warnings remain as history, while a successful output is never replayed. Core attempts admission before reporting success; a rejected admission leaves every row unchanged and returns an error to the operator.
