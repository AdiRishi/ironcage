...
This chapter owns CommBank import, source preservation, transaction identity, coverage, categorization, transfers, and spend analysis. [Data](/technical/data) owns shared storage conventions. [AI](/technical/ai) owns capability dispatch and decision records. [Tax](/technical/tax) owns the tax interpretation of bank rows.
...
- An archived statement cannot add history; a future enabled parser must first reconcile it from opening balance to closing balance.
...
- Structured-import preview is stateless. No import file or row is stored until confirm succeeds.
- Statement archival stores only the immutable PDF and archive metadata; it creates no transaction, observation, or coverage.
...
A **source observation** is one bank-supplied representation of that movement. In the enabled profile, a CSV row and its paired OFX row are separate observations that link to one bank transaction. A future certified statement parser may add another observation.

An **import bundle** is the set of files submitted together for one account and source window. The enabled CommBank bundle contains CSV and OFX.

A **statement archive** is one original PDF preserved against the operator-selected account. It is source material, not an import bundle, and does not prove transaction history by itself.
...
flowchart LR
    ...
    O --> T
    P["Statement archive"] --> R["Immutable PDF"]
    R -.->|"after profile certification"| S["Statement source observations"]
    S -.-> T
    T --> A["Categories, transfers, analysis, tax"]
```

The solid paths are phase one. The dotted paths show the later, fixture-gated backfill. A source observation is never itself counted as spending.

The enabled structured import runs in the order below, and nothing is stored until the last stage commits:

```mermaid
flowchart LR
    U["Upload CSV + OFX"] --> S1["§1 Profile<br/>account + source identity"] --> S2["§2 Parse + reconcile<br/>the pair"] --> S4["§4 Normalize"] --> S5["§5 Active dedupe<br/>bundle, ID, balance, content"] --> S6["§6 Coverage"] --> S7["§7 Preview → confirm<br/>one Postgres commit"]
```
...
The operator selects the expected account during upload. OFX identity must map to that account and no other. CSV never chooses the account because the observed CSV files contain no identity. Archival records the account selected by the operator; a later statement parser must independently prove the PDF's account identity before adding observations.

The enabled source profile is:

- `cba-netbank-paired-v1` for an exact CSV/OFX pair.

`cba-offset-statement-v1`, Mastercard statements, and home-loan statements remain disabled until representative PDFs pass their fixture gates. PDFs can still be archived without affecting transactions or coverage.
...
## 3. Statement archival and future backfill

Phase one archives statement PDFs as immutable, content-addressed source material. Archival does not extract text, create observations or transactions, or claim coverage. The parsing design below is deliberately dormant until a committed redacted overlap fixture proves it.
...
AnyDoc is an extractor, not the financial parser. The pinned native package runs in an isolated compute container with network access disabled and no database credentials. The image is packaged now, but the importer does not call it while every statement profile is disabled. Once a profile passes its fixture gate, core will validate the returned Markdown with that profile's deterministic state machine.

A future confirmed statement import stores three artifacts:
...
When statement parsing is enabled, an image-only or extraction-empty PDF will remain archived and block backfill until an approved extraction path exists. The first release does not send private statements to a hosted OCR service.

### Planned statement state machine

The parser processes extracted blocks in page order. It is a state machine, not a linear pass — the table state loops once per transaction, and a transaction accumulates continuation lines until its amounts complete it:
...
Page headers and footers are the only blocks the table state may skip, and only on an exact profile match — nothing else is silently ignored.
...
sequenceDiagram
    ...
    Note over Core,PG: preview writes neither Postgres nor R2
    ...
    alt bytes changed
        ...
        Core->>R2: content-addressed source artifacts, first
        Core->>PG: one transaction — import, files, observations,<br/>transactions, links, balances, coverage, feed event
        Note over R2,PG: a failed transaction leaves only unreferenced<br/>immutable objects — a retry reuses their digest keys
        Core-->>App: confirmed
    end
```

Preview performs the full parse and match in memory against a consistent database snapshot. It writes neither Postgres nor R2.
...
If the database transaction fails, the R2 artifacts are unreferenced immutable objects. A retry reuses their digest keys. A periodic job removes unreferenced objects after the retention delay owned by [Data](/technical/data).
...
Statement archival is a separate mutation with no preview. Core validates the selected account and the PDF boundary, writes the content-addressed original to R2, then inserts archive metadata and one feed event. The `(account, digest)` constraint makes a repeated upload return the existing archive without creating transactions or coverage.
...
| `bank_statement_archives`    | account, original display name, byte digest, byte size, immutable R2 key, archived time; digest unique per account              |
...
The application boundary keeps structured imports and archival as separate operations:
...
interface ArchiveBankStatementRequest {
  readonly accountId: BankAccountId;
...
| Statement source                   | archive one PDF now; import only under an account-specific fixture-proven profile      | importer              | decided  |
...
- [x] Package native AnyDoc 0.1.7 in the isolated, network-disabled compute image.
- [ ] Record the deployed image digest with each extraction when statement parsing is enabled.
- [x] Archive statement PDFs idempotently without creating transactions or coverage.
...
- [x] Create the source-file, observation, canonical-transaction, link, balance, coverage, and statement-archive tables.
