...
- **`/capital`** — `CapitalEvent` (deposit | withdrawal | inter-venue transfer | fund | reduce | return, with amounts, venue, note, evidence reference; physical movements carry a pending → settled lifecycle, settled by reconciliation matching), `TransferRequest` (amount, source, destination, rail, status), `SystemCageConfig`.
...
| `capital_events`                                        | append-only + status | the capital ledger; physical movements pending → settled; cash derived per venue    |
