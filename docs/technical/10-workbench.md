...
1. **Sort** ascending by `venue`, then `instrument`, then `timeframe`, each compared byte-wise over its UTF-8 encoding, then by `close_ts` as an integer.
2. **Render each candle** as one line of nine fields joined by `|` (U+007C) and terminated by `\n` (U+000A): `venue|instrument|timeframe|close_ts_ms|open|high|low|close|volume`.
3. **Render decimals** in plain notation: no exponent, no leading `+`, one leading zero at most, trailing zeros after the point stripped, no point on an integral value, and zero as `0`.
4. **Concatenate** the lines with no separator, no header, and no trailing byte beyond the final `\n`.
5. **Hash** the concatenation with SHA-256 and render it as lowercase hex. An empty candle set is a manifest-validation failure, not a hash.
...
1. For each evaluation fold, compute the candidate's return net of the manifest's cost model, minus the mandate's primary benchmark return over the same fold. Buy-and-hold of the universe is always a benchmark.
2. Those `k` fold excesses are the observations. `k` **must** be at least 6.
3. The stitched out-of-sample result **must** contain at least **30 closed trades**. Below the floor the gate fails whatever the p-value, because a handful of trades supports no inference.
4. Apply a one-sided Student's t-test on the mean excess, with null hypothesis "mean excess ≤ 0" and `k − 1` degrees of freedom.
5. The gate passes when `p < α`. `N` is floored at 1, so a first-ever trial faces `α = 0.05`.
6. A companion risk threshold applies independently: the stitched out-of-sample maximum drawdown **must not** exceed **1.25 ×** the benchmark's drawdown over the same window.
...
This bucket serves the design-time proposal path only. Runtime capability outputs travel through the same queue and land directly in Postgres; they never touch the bucket ([AI](./07-ai.md)).
...
The proposal row, its bundle hash, and every verdict live in the schemas specified in [Data](./03-data.md). The capability that authors proposals is specified in [AI](./07-ai.md); it may read everything here and may start runs, and it must not start a live sleeve, skip a gate, or write a counter.
