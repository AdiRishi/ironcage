---
title: App
description: How the observatory renders, updates live, and authenticates — the route tree, data paths, the feed wire protocol, research chat, the shell, and auth.
sidebar:
  icon: monitor
---
...
This chapter owns the route tree, data paths, feed wire protocol, research chat, shell, and authentication. [Data](/technical/data) defines the feed record and taxonomy. [Contracts](/technical/contracts) defines the API catalog and error taxonomy. [Operations](/technical/operations) defines deployment.
...
- routes/
  - \_\_root.tsx shell: mode, halt-all, criticals, override banner, connection
  ...
  - money/
    ...
    - transactions.tsx the ledger — every row, its category, who filed it; the drill-down from every aggregate
...
This section is the single home of the frame protocol. Other chapters reference it; they must not restate it. The feed record the protocol carries is defined in [Data](/technical/data).
...
<Steps>
  <Step title="Mark replaying">
    Before its first await, mark the socket as replaying and start an in-memory live-event buffer.
  </Step>
  <Step title="Read the record">
    Read a Postgres high-water event ID, then read and cap the rows in `(since, high_water]`.
  </Step>
  <Step title="Merge and go live">
    Without another await, merge those rows with the buffered live events, sort and de-duplicate by
    event ID, send the result, transition the socket to live, and send `Ready`.
  </Step>
  <Step title="Push by state">
    A live push checks the per-socket state: it appends while replaying and sends immediately while
    live.
  </Step>
</Steps>
...
The race the four steps close, on one wire — a live push landing mid-replay is buffered, not lost and not reordered:

```mermaid
sequenceDiagram
    participant C as Client
    participant F as Feed actor
    participant PG as Postgres
    participant O as Originating actor
    C->>F: Subscribe(since = last applied ID)
    F->>F: mark socket replaying, start buffer
    F->>PG: high-water ID, then rows in (since, high_water]
    O-->>F: live push (a fact just committed)
    F->>F: buffered — socket is still replaying
    PG-->>F: replay rows
    F->>F: merge + sort + dedupe by event ID, no await
    F->>C: events, then Ready(replayed = n)
    F->>F: socket is live
    O-->>F: next live push
    F->>C: sent immediately
```
...
`/api/conversation/*` validates the Access session and the request's Origin, then forwards over `AGENTS`. Response bodies stream unbuffered through the single hop. Agents hold no mutation path into the product: their only inputs to core are the read-only agent API and the queue, and their model spend is capped at the AI Gateway. [AI](/technical/ai) defines both.
