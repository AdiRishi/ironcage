...
- **Determinism is a test**: the same backtest manifest run twice must produce byte-identical results; this runs in CI and *is* the same check gate 2 of the pipeline performs in production ([Workbench](./07-workbench.md)).
