...
<Steps>
  ...
  <Step title="Consumption">
    ...
    own surface: categorization output is applied to Money's ledger.
  </Step>
</Steps>
...
```mermaid
flowchart TB
    subgraph sleeve["One sleeve, per evaluation window"]
        real["The real book —<br/>what actually happened"]
        act["'Actual' leg — the chosen posture,<br/>re-simulated under the fill model"]
        idn["Identity leg — every capability at its<br/>no-AI identity, same strategy, cage, data"]
    end
    act --> D["pnl, drawdown, and cost deltas —<br/>simulation minus simulation"]
    idn --> D
    D --> RPT["Scorecard"]
    real -.->|"reported beside the deltas,<br/>never inside them"| RPT
```
...
Money's transaction categorization reuses the capability runtime — the same queue path, authoritative validation, and decision records — as the system's first batch-triggered capability. A categorization run answers one import batch: its run ID anchors on the bundle digest and batch index, and its output carries no validity window. It is not a sleeve capability. It has no sleeve, no mandate, and no baseline book; it lives in Money's own workflow and its registry entry is Money-owned. Operator-created rules short-circuit the model: a rule-matched transaction never reaches an agent. Core applies a validated answer on delivery to every row still uncategorized, and the operator's later corrections are recorded as kept or overridden — the corpus that measures the capability's precision. The application rule is owned by [Money](/technical/money).
...
| Gateway spend cap                               | $50 over a sliding one-week window                                                                       | gateway config                                | decided                     |
...
| Categorization application                      | on delivery, to rows still uncategorized; kept/overridden corrections form the corpus                    | operator ([Money](/technical/money))          | decided                     |
