...
A transfer is created when the operator records a movement or when a capital act needs money moved. The system never initiates a bank or venue withdrawal; it requests, records, and watches.

```mermaid
stateDiagram-v2
    [*] --> pending: operator records, or a capital act requests
    pending --> settled: unique exact match within 3 days, or explicit reference
    pending --> flagged: window passes, or the amount differs
    pending --> dismissed: operator dismisses — cancels the act awaiting it
    flagged --> settled: operator's hand only
    flagged --> dismissed: operator's hand only
```
...
Armed by ceremony with a typed confirmation phrase. It covers one limit, one sleeve, one action class, with a stated maximum action size. It is consumed by a single use, applied by compare-and-set inside the verdict's transaction, so two concurrent evaluations cannot both spend it.

```mermaid
stateDiagram-v2
    [*] --> armed: ceremony, typed confirmation phrase
    armed --> active: 15-minute cooling-off elapses
    active --> consumed: one use — compare-and-set in the verdict's transaction
    active --> expired: 24 hours after activation
    armed --> voided: any new mandate version for the sleeve
    active --> voided: any new mandate version for the sleeve
```
