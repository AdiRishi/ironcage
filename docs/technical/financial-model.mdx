...
Amounts use signed integer minor units with an explicit currency. Parsing converts decimal strings directly to minor units, without binary floating-point multiplication. Fractional intermediate calculations use decimal or rational arithmetic and round only at a defined output boundary.

Wire values and aggregation reject values outside the supported exact integer range. A currency's exponent is explicit. Historical Australian-dollar totals use the bank's booked Australian-dollar amount, not a later exchange rate.
...
Bank posting dates are calendar dates, not midnight UTC timestamps. Import times and command times are UTC instants. Display and calendar grouping use Australia/Sydney by default. A selected timezone is part of an analysis definition.
...
The initial storage design uses relational D1 tables for these records, plus indexes on account and posting date, source identity, event membership, and review state. Numeric totals remain exact across queries.

Financial writes use an expected revision and a unique command ID. A transaction atomically records the change, audit entry, new revision, and dependent dispatch record. A repeated command returns its stored result. A concurrent stale command receives a conflict with the current version.

D1 batches provide the atomic boundary for related SQL statements. Revision checks are enforced inside that boundary, with every dependent write gated on the accepted command. An optimistic update that affects no rows cannot be followed by unconditional financial writes. [D1 batches](https://developers.cloudflare.com/d1/worker-api/d1-database/#batch)
