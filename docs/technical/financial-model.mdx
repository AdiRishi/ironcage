...
Stage 1 establishes identity, source evidence, exact values, unresolved events, and publication/recovery invariants. Stage 3 adds interpretation, relationships, and the measures required by correction previews. Stage 4 adds general analysis queries and immutable results; stage 5 adds ordinary-cost and schedule measures. Apply each invariant when its owning behavior exists.
...
The [database schema](/technical/database-schema) specifies exact tables, columns, constraints, and indexes. [Shared contracts](/technical/contracts) specifies their wire representations. [Calculation rules](/technical/calculation-rules) defines event construction and metric algorithms.
...
The [database schema](/technical/database-schema) owns the exact field, nullability, and constraint definitions for these records.
...
Document processing, model calls, R2 transfers, and Workflow dispatch occur outside financial write transactions. Statement, lock, and transaction deadlines bound resource use. Dispatch status changes have their own lease operations and do not advance the financial revision.
