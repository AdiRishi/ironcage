...
description: The decision ceremony, modes, the operator's controls, the break-glass override, and the integrity behaviors that keep the system honest under failure.
sidebar:
  label: Operations
...
<Steps>
  <Step title="The evidence is presented">
    The trial report, capability scorecards, gate results, incident report, or diff that informs
    this decision is shown at the moment of choosing — and the record notes what was shown.
  </Step>
  <Step title="The change is stated plainly">
    What will be different afterward, in one paragraph, including the risk added where there is any.
  </Step>
  <Step title="A reason is written where risk increases">
    Mandate changes, promotions, and overrides require the operator's words; risk-reducing decisions
    never demand an essay.
  </Step>
  <Step title="The decision is recorded beside its evidence, permanently">
    Approve and reject alike, with timing. A decision the operator can't later reconstruct — what
    was known, what was chosen, why — is a bug in this ceremony.
  </Step>
</Steps>
...
Sleeve states are specified in [Sleeves](/product/sleeves). Above them, the system as a whole is in exactly one mode, visible in the shell on every screen ([product introduction](/product#the-shell)):

- **Running** — normal operation.
- **Halted** — nothing trades anywhere. Entered automatically by the system cage ([Portfolio](/product/portfolio) — total drawdown or exposure breach, or a failure the system cannot attribute to a single sleeve), or manually by the operator's halt-all. Leaving system Halted is always an operator action from the incident report.

## The operator's controls

**Risk-reducing actions are always available and never gated.** From any relevant view — including every row of the current book ([Portfolio](/product/portfolio)) — one click plus a confirm:
...
- A system-level halt, or a critical event with real positions open, additionally triggers the one interruption ([product introduction](/product#the-one-interruption)) — the contentless "Ironcage needs you" notification.
