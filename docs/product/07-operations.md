...
- **Scope**: an override names one specific limit on one sleeve and permits one class of action past it (e.g. "allow one entry in BTC above the position cap"). An override may also lift a lock a capability wrote — an event veto's lock, for example — through the same ceremony: the operator outranks the AI, and the ceremony preserves the evidence. It cannot disable auditing, cannot touch the always-on risk-reducing controls, cannot affect any other sleeve, and cannot suspend the system cage.
