---
title: Money
description: The operator's banking picture — bank import, deduplication, coverage, categorization, and spending analysis. Reads money; never moves it.
sidebar:
  icon: banknote
---
...
For older history, one upload contains a CommBank offset statement PDF. Money extracts its text, verifies the account and statement period, reconstructs every transaction through the running-balance chain, and proves the opening balance, closing balance, and summary totals before preview. A statement never overrides a complete structured-export window.

The import flow is:

<Steps>
  <Step title="Select">A CSV/OFX pair for recent history or one supported statement PDF.</Step>
  <Step title="Preview">
    The account, source window, coverage effect, balances, and row verdicts.
  </Step>
  <Step title="Resolve">Any rows the system cannot identify unambiguously.</Step>
  <Step title="Confirm">The import.</Step>
  <Step title="Inspect">The ledger — every new row filed, and by whom, open to change.</Step>
</Steps>

Nothing from an import is stored before confirmation. The preview reports source rows, new transactions, known duplicates, ambiguities, and coverage gaps. A file for the wrong account is rejected before row matching begins. A structured export that reaches NetBank's observed 600-row ceiling is treated as truncated and cannot claim complete coverage.

Uploaded CSV, OFX, and PDF bytes are request-scoped. Statement extraction may use ephemeral container storage, but the uploaded bytes and extracted Markdown are discarded after processing. Confirmation retains the canonical transactions, source observations, balance evidence, import digest, and minimal source metadata needed to explain and safely replay the result.
...
The record distinguishes a bank transaction from the source observations that support it. A CSV/OFX pair provides two observations of one transaction. An overlapping statement may provide another. Each observation retains the source fields needed to explain the transaction; it does not retain the uploaded file.

Deduplication uses the strongest evidence the bank supplies, in order:

1. The exact file or bundle digest makes an identical re-upload idempotent.
2. A proven bank transaction identifier is decisive within its account.
3. A posted date, exact amount, and running balance identify rows on accounts that publish a balance for each transaction.
4. Ordered occurrences distinguish repeated rows when no balance or identifier exists.
5. A supported statement profile uses its totals, balance sequence, account identity, and row order to align observations with existing transactions.
...
Older history comes from supported statement imports. The operator should download and import all available statements while NetBank still exposes them because that window moves forward over time.
...
Every transaction is filed the moment it arrives, the way a bank files it: rules first, then AI for whatever no rule recognized. The operator's job is to inspect and correct, never to file by hand as a matter of course.

Rules run first and are decisive. An AI run answers every row a rule left uncategorized, and its answer is applied as that row's category, with a one-line rationale kept alongside. The AI never touches a row a rule or the operator has already filed. A row stays uncategorized only when the AI run failed or produced no answer for it, and those rows are surfaced first in the ledger so they get a hand.

Every assignment records whether it came from a rule, the AI, or the operator, and the ledger shows that provenance on every row. Correcting an AI-filed row records whether the operator kept or overrode the AI's category — the correction corpus that measures the AI's precision over time. Correcting a payee can create a visible rule for future transactions. The underlying source narrative never changes when a category changes.
