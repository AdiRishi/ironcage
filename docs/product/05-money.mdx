...
For older history, Money preserves statement PDFs immediately without adding their rows to the record. After the parser for an account's statement layout passes its fixture tests, a later backfill can reconcile the statement from its opening balance through every row to its closing balance. A statement never overrides a complete structured-export window.

The structured import flow is:

<Steps>
  <Step title="Select">The CSV and OFX for one recent-history bundle.</Step>
  <Step title="Preview">
...
Nothing from a structured import is stored before confirmation. The preview reports source rows, new transactions, known duplicates, ambiguities, and coverage gaps. A file for the wrong account is rejected before row matching begins. A structured export that reaches NetBank's observed 600-row ceiling is treated as truncated and cannot claim complete coverage.

Statement archival is intentionally shorter: select an account and PDF, then archive it. The original bytes and digest are stored, but no preview, parsing, transaction, or coverage claim occurs.
...
The record distinguishes a bank transaction from the source observations that support it. A CSV/OFX pair already provides two observations of one transaction. Once a statement profile is certified, an overlapping statement may provide another. Every imported raw narrative and source file remains attached to that transaction for inspection.
...
5. Once a statement profile is enabled, its totals, balance sequence, account identity, and row order align its observations with existing transactions.
