...
AI-originated changes arrive through the same door: a proposal ([Sleeves](./02-sleeves.md)) that has passed its gates reaches the operator as a pre-filled mandate change — diff, rationale, and gate results attached — and is approved or rejected on the same review screen with the same recording. The AI never gets a second, quieter path into the mandate.
