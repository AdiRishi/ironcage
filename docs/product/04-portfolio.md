...
Ironcage never calls a withdrawal, whitelist, or transfer API. Kraken's key proves that boundary through permissions. Alpaca's broad credential does not publish the same guarantee, so the wealth sleeve stays non-live until the account-level transfer-surface gate closes. A failed gate reopens venue selection.

Every physical transfer, whether bank to venue, venue to bank, or venue to venue, is still performed by the operator. The product's job is **compute, request, detect, settle**.

- **Transfer requests.** When capital needs to be somewhere it isn't, the system writes a **transfer request** with the amount, source, destination, and a direction-valid rail. Eligible Kraken AUD deposits use Osko with the account's generated PayID. Withdrawals use the currently enabled bank or Osko method because PayID withdrawals are not supported. Alpaca uses the funding route proven for the actual account. A request is a recommendation: fulfil, adjust, or dismiss it. Every outcome is recorded.
