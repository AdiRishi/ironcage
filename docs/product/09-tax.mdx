...
The engine's shape, which the sections below walk in order — anything the rules cannot classify with confidence exits to the review queue instead of entering the figures:

```mermaid
flowchart LR
    S["Sources<br/>sleeves · exchanges · wallets<br/>broker · bank · manual"] --> L["One ledger of<br/>canonical events"]
    L --> R["Australian rules<br/>CGT · income · forex · foreign"]
    R --> V["Verification<br/>balances · gaps · 1042-S · oracle"]
    V --> F["The FY report,<br/>shaped as myTax asks"]
    L -.->|"unrecognized or<br/>low-confidence"| Q["Review queue —<br/>flagged, never guessed"]
```
