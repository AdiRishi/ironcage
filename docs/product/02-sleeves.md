...
- **AI capabilities** — which capabilities from the registry this sleeve holds, with their per-sleeve parameters (below).
...
Every capability output — every assessment, veto, tightening, and proposal — opens its decision record ([Activity](./03-activity.md)): what the model was asked, what it decided, and why, with a link into the platform's full trace while the platform retains it.

### Profiles (shorthand, not structure)

Named capability bundles exist purely as vocabulary: **Clockwork** = no capabilities (pure rules — the long-term wealth sleeve). **Advised** = the run-time throttle bundle plus observers. **Assisted-design** = Advised plus proposal capabilities. **Piloted** = includes the trade proposer. The living view always shows the actual capability list; the profile name is a summary, never the mechanism.
