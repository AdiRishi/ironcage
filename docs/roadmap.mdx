...
What it delivers: fixture-proven paired CommBank CSV/OFX import and CommBank offset-statement PDF import with overlap dedupe, source metadata, balance reconciliation, and account-level coverage. It also delivers the categorization capability with its review queue and rules, monthly/trends/recurring/anomaly analysis, savings suggestions, the monthly spending report, and external balances feeding a first whole-of-wealth view. QIF remains unsupported because it adds no identity or reconciliation evidence. Upload bytes and extracted statement text are transient processing inputs and are discarded after the request.
