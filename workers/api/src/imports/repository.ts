...
import { Import, ImportInput, ImportSource, FinanceError } from "@repo/contracts/finance";
...
export class ImportRepository extends Context.Service<
  ImportRepository,
  {
    readonly list: () => Effect.Effect<ReadonlyArray<Import>, FinanceError>;
    ...
    readonly fail: (
      input: typeof ImportInput.Type & { failure: { message: string } },
    ) => Effect.Effect<void, FinanceError>;
  }
>()("@repo/api/imports/ImportRepository") {
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const fields = sql`i.id, i.source_file_id AS "sourceFileId", i.account_id AS "accountId", s.file_name AS "fileName", i.format, i.status, i.summary, i.failure, i.version, i.created_at::text AS "createdAt"`;
      const list = Effect.fn("Imports.list")(function* () {
        return yield* sql`SELECT ${fields} FROM imports i JOIN source_files s ON s.id = i.source_file_id ORDER BY i.created_at DESC, i.id DESC LIMIT 100`.pipe(
      ...
      const fail = Effect.fn("Imports.fail")(function* ({
        importId,
        failure,
      }: typeof ImportInput.Type & { failure: { message: string } }) {
        yield* sql
          .withTransaction(
            Effect.gen(function* () {
              yield* sql`SELECT pg_advisory_xact_lock(1)`;
              yield* sql`UPDATE imports SET status = 'failed', failure = ${sql.json(failure)}, version = version + 1, updated_at = now() WHERE id = ${importId} AND status = 'processing'`;
            }),
          )
