...
// Everything the tools registered in a turn, or in the reads a briefing is written from, in
// order.
export type Evidence = {
  ...
  // The number the next figure or record takes.
...
export class TurnEvidence extends Context.Service<
  TurnEvidence,
  {
    ...
    // A turn stores each step as soon as it is taken, so the conversation shows it while the
    // turn runs.
...
>()("@repo/analyst/evidence/TurnEvidence") {
  ...
  static readonly layer = (api: LedgerApi, turnId: TurnId) =>
    Layer.effect(
      TurnEvidence,
      Effect.gen(function* () {
        const sql = yield* SqlClient.SqlClient;
        return yield* make(api, {
          first: yield* readNextReference(turnId),
          owner: turnId,
          step: (step) =>
  ...
  // A briefing's figures and records number from 1, and nobody watches its reads as steps.
  static readonly briefing = (api: LedgerApi, month: YearMonth) =>
    Layer.effect(
      TurnEvidence,
      make(api, { first: 1, owner: `briefing/${month}`, step: () => Effect.void }),
    );
}

type LedgerApi = Pick<Api, "getSettings" | "listAccounts" | "getFactsStatus">;

// `first` is the number the first figure or record takes, and `owner` names the turn or
// briefing the evidence is for, which a proposal's ID comes from.
const make = Effect.fnUntraced(function* (
  api: LedgerApi,
  {
    ...
    readonly step: TurnEvidence["Service"]["step"];
  },
) {
  const [settings, accounts, facts] = yield* Effect.all(
    [api.getSettings(), api.listAccounts(), api.getFactsStatus()],
  ...
  return TurnEvidence.of({
    ...
    propose: (proposed) =>
      Ref.modify(state, (current) => {
        ...
        const id = ProposalId.make(v5(`${owner}/proposals/${place}`, v5.URL));
        const proposal = {
          id,
          ...proposed,
          status: "pending",
