...
it("two identical transactions in a new file create two postings", () => {
  expect(matchObservations(source, [row(rowOne), row(rowTwo)], [])).toEqual({
    assignments: [
...
it("two identical transactions in another complete file support the two existing postings", () => {
  expect(
    matchObservations(source, [row(rowOne), row(rowTwo)], [posting(first), posting(second)]),
  ).toEqual({
    assignments: [
...
it("a single coffee cannot choose between two identical postings", () => {
  expect(matchObservations(source, [row(rowOne)], [posting(first), posting(second)])).toEqual({
    assignments: [],
    questions: [
      {
        kind: "duplicate",
...
it("unequal groups accept a single corroborated pair", () => {
  expect(
    matchObservations(
      source,
      [row(rowOne)],
      [posting(first, { ...coffee, description: "Bus" }), posting(second)],
    ),
  ).toEqual({
...
it("a single statement row can support a single structured posting despite different wording", () => {
  expect(
    matchObservations(
      source,
      [row(rowOne, { ...coffee, description: "Card purchase at cafe" })],
      [posting(first)],
    ),
  ).toEqual({
...
it("a reparse changing an accepted source row asks for a bank-field correction", () => {
  ...
  expect(
    matchObservations(
      source,
      [row(rowOne, { ...coffee, description: "Different reading" })],
      [existing],
    ),
  ).toEqual({
    assignments: [],
...
it("an unchanged source row stays linked when another source supplies the display wording", () => {
  ...
  expect(matchObservations(source, [row(rowOne)], [existing])).toEqual({
...
it("a user-declared distinct transaction remains distinct on a reparse", () => {
  ...
  expect(matchObservations(source, [input], [posting(first), accepted])).toEqual({
...
it("the user's chosen posting takes precedence over automatic pairing", () => {
  ...
  expect(matchObservations(source, [input], [posting(first), selected])).toEqual({
    assignments: [{ observationId: rowOne, postingId: second, method: "user" }],
    questions: [],
  });
});
