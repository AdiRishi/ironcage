...
// A minimal hand-assembled PDF proves the WebAssembly extractor initializes
// and converts inside workerd; real-statement extraction quality is proven
// against the private corpus, which never enters the repository.
...
test("the statement extractor converts a PDF inside workerd", async () => {
  const stub = env.STATEMENT_EXTRACTION.getByName("test");

  const result = await stub.extract(minimalPdf());
  expect(result.extractor).toEqual({ package: "@firecrawl/anydoc-wasm", version: "0.1.7" });
